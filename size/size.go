...
// FromString parses a Size from a string.  A Size is an unsigned decimal number with
...
func FromString(s string) (Size, error) {
	// ([0-9]*(\.[0-9]*)?[a-z]+)+
	if s == "" {
		return 0, errors.New("invalid Size: empty")
	}
...
func suffixSize(suffix string) (Size, error) {
	...
	case "pib":
		return Pebibyte, nil
	default:
		return 0, fmt.Errorf("unknown Size unit suffix: %q", suffix)
	}
}
